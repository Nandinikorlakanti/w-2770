    ...
    <div className="space-y-6">
      ...
      <div className="relative">
        <Textarea
          ...
          className="min-h-[120px] text-base bg-gradient-to-br from-white/80 to-indigo-50/80 dark:from-slate-800/80 dark:to-slate-700/80 backdrop-blur-sm border-2 border-indigo-200/50 dark:border-slate-600/50 focus:border-gradient-to-r focus:from-indigo-400 focus:to-purple-400 rounded-2xl shadow-lg transition-all duration-300 resize-none placeholder:text-slate-400 dark:placeholder:text-slate-500"
          disabled={isLoading}
        />
        ...
        {preview && (
          <div className="absolute top-3 right-3 bg-gradient-to-br from-white/95 to-indigo-50/95 dark:from-slate-800/95 dark:to-slate-700/95 backdrop-blur-md rounded-xl p-3 border border-indigo-200/50 dark:border-slate-600/50 max-w-xs shadow-xl">
            ...
            <div className="space-y-2">
              <div className="text-sm font-semibold text-slate-700 dark:text-slate-200 truncate">{preview.name}</div>
              {preview.assignee && (
      ...
      <Button
        ...
        className="w-full h-14 text-base font-semibold bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:from-indigo-600 hover:via-purple-600 hover:to-pink-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] rounded-2xl shadow-lg hover:shadow-xl"
      >
        {isLoading ? (
          <>
      ...
      <div className="text-center text-xs text-slate-500 dark:text-slate-400">
        Press <kbd className="px-2 py-1 bg-slate-200 dark:bg-slate-700 rounded-lg text-xs font-mono">Cmd+Enter</kbd> to add task
      </div>
    </div>
  );
}
